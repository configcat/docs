...
The ConfigCat backend will take the Dashboard preference as primary. So if you have EU Only selected on the Dashboard but you did not pass the proper `dataGovernance` parameter to the SDK. In this case your config JSONs will only
published to the EU CDN nodes. And every request is being made from your application to any node outside the EU the request will be redirected automatically to the EU CDN. Recommended to deploy a fix since response times will be significantly longer.
