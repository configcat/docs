...
As part of the scanning operation, the CLI gathers all deleted feature flag and setting from the last 180 days and looks for their usage
in your source code. When it finds a reference for a deleted feature flag or setting, it prints a warning that lists their keys.
