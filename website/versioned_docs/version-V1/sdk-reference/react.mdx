...
<p>[ConfigCat React SDK on GitHub](https://github.com/configcat/react-sdk)</p>
...
When calling `useFeatureFlag()` or `WithConfigCatClientProps.getValue()`, the _ConfigCat SDK_ downloads the latest config data from the ConfigCat CDN only if it is not already present in the cache, or if the cache has expired.
