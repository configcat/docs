...
:::info
This documentation applies to the **v10.x version** of the ConfigCat Swift SDK. For the documentation of the latest release, please refer to [this page](../../../docs/sdk-reference/ios.mdx).
:::
...
Snapshots are created from the actual state of the SDK; therefore it's crucial to know whether the SDK has valid feature flag data to work on. 
To determine whether it's safe to create snapshots, the SDK provides an `onReady` [hook](#hooks). 
It accepts a state enum parameter to give details about the SDK's initialization state.

Snapshots created before the SDK reaches its ready state are considered empty, thus they will return default values for feature flag evaluation requests.
...
The SDK's state is also accessible via the `waitForReady()` awaitable method, which asynchronously waits for the `onReady` hook to fire and returns with the SDK's initialization state.
...
in the background. For other polling modes, you'll need to manually initiate a cache refresh by invoking `forceRefresh()`.
:::
