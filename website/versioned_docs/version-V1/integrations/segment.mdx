---
id: segment
title: Segment - Send feature flag analytics to Segment
description: ConfigCat Segment integration. This is a step-by-step guide on how to send feature flag analytics to Segment.
---
...
There are two available integration oppurtunities between ConfigCat and Segment:
- [Sending feature flag change events to Segment](#changeevents)
- [Sending feature flag evaluation analytics to Segment](#analytics)
...
<img src="/docs/assets/segment/changeevent.png" className="zoomable" alt="Segment featureFlagsChanged event" />
...
4. Click on Segment's CONNECT button and set the Twilio Segment Write Key acquired while adding the ConfigCat source in Segment.
5. OPTIONAL - Set the proper server of your Segment account. <a href="https://segment.com/docs/guides/regional-segment/" target="_blank" rel="noopener noreferrer">More about Segment servers</a>.
6. You're all set. Go ahead and make some changes on your feature flags then check your events in Segment.
...
2. Click on Segment's DISCONNECT button.

### Event details

Every feature flag change event sent to Segment by ConfigCat has:
...
Ensures that feature flag evaluations are sent into the Segment ConfigCat source. With this integration, you can have advanced analytics about your feature flag usages, A/B test results in any of Segment's destinations.

### Setup

1. **Add ConfigCat source in Segment:** Add the ConfigCat source in Segment.
...
2. **Configure SDKs:**
   ...
   - **Segment SDK:** Set up with the Twilio Segment Write Key acquired while adding the ConfigCat source in Segment.
...
While our documentation primarily provides code examples for languages that Segment natively supports and has an official SDK, 
you can integrate with other languages by sending an event to Segment with a third-party SDK or with using the <a href="https://segment.com/docs/connections/sources/catalog/libraries/server/http-api/" target="_blank" rel="noopener noreferrer">Segment's HTTP API source</a>. 

1. **Subscribe to the FlagEvaluated hook** in the ConfigCat SDK.
2. **Send an event to Segment** using the `featureFlagEvaluated` event name. Include the following event properties:
...
:::note
For Text feature flags with lengthy values (e.g., JSON), send the `variationId` instead of the `value` as the `value` to Segment.
...
4. Deploy your application and wait for feature flag evaluations to happen so feature flag evaluation events could be sent to Segment.
5. Add the preferred destination in Segment for your source and configure your mappings. 
...
<img src="/docs/assets/segment/segment.png" className="zoomable" alt="Segment event debugger" />
