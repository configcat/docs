...
ConfigCat provides a seamless integration with Amplitude, a product analytics tool that helps you understand user behavior 
and make data-driven decisions. With this integration, you can monitor your feature flag changes in Amplitude, and send 
feature flag evaluation analytics to Amplitude Experiments.

There are two available integration oppurtunities between ConfigCat and Amplitude. You can:
- [Monitor your feature flag change events in Amplitude with Annotations](#annotations)
- [Send feature flag evaluation analytics to Amplitude Experiments](#experiments)
...
2. **Send an event to Amplitude** using the `$exposure` event name. Include the following event properties:
   ...
   - `variant`: the evalated feature flag's value or the variationId from the FlagEvaluated hook's EvaluationDetails
   - `variation_id`: the evalated feature flag's value or the variationId from the FlagEvaluated hook's EvaluationDetails
