...
The `config JSON` is cached on the ConfigCat CDN. If there is no change, the ConfigCat CDN will respond with a `304 Not Modified` response. If there is a change, the ConfigCat CDN will respond with a `200 OK` response, and the content of new `config JSON`.
...
The [CDN proxy](/advanced/proxy/endpoints/#cdn-proxy) feature allows you to proxy/cache config JSON downloads, and the ConfigCat SDKs can be routed to the ConfigCat Proxy running in your own infrastructure greatly reducing network traffic.
