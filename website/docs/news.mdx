...
We're excited to introduce **Change Requests** — a way to prepare feature flag changes without publishing them right away.

You can propose changes for later manual publishing, or schedule them to be applied automatically at a specific time. Another person can review and approve the change before it is published.
...
Change Requests are available for **V2 configs only** and are not supported for V1 configs.

Read more in the [Change Requests documentation](./change-requests.mdx).
