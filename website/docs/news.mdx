...
<p>
  <img src="/docs/assets/news/dotnet-sdk-v10-192dpi.png" alt="ConfigCat .NET SDK" width="400" height="300" decoding="async" loading="lazy" />
</p>
...
However, the most exciting addition is probably the **new integration package**, `ConfigCat.Extensions.Hosting`.
...
- **Automatic registration in the standard Dependency Injection container** (`Microsoft.Extensions.DependencyInjection`).
  ...
  Even multiple client instances are supported out-of-the-box via [keyed services](https://learn.microsoft.com/en-us/aspnet/core/fundamentals/dependency-injection?view=aspnetcore-8.0#keyed-services).
...
.NET runtimes, namely .NET Framework versions 4.6.1 and below. For these runtime versions, the SDK's v9.x line can still
be used as long as [Config JSON schema V6](https://github.com/configcat/config-json/tree/main/V6) is supported.
However, please note that v9.x will not be actively maintained.

## Feature Flags for Startups: Meet the ConfigCat Startup Program

#### Jun 17, 2026

Building a startup means shipping fast, testing ideas quickly and changing direction as you learn. But fast releases should not mean risky releases.

That's why we created the **ConfigCat Startup Program**: a startup-friendly way for early-stage teams to use production-ready feature flags from day one.

Eligible startups can get:

- 50% off Pro or Smart annual plans in Year 1
- 25% off Pro or Smart annual plans in Year 2
- Early access to upcoming ConfigCat features

Learn more about the Startup Program and apply here: [ConfigCat Startup Program](https://configcat.com/startup-program/).
...
We have added two new Public Management API endpoints that supersede the existing audit log endpoints:
- List Audit log items for Product (V2)
- List Audit log items for Organization (V2)

The new V2 endpoints introduce pagination support, giving you finer control over the amount of data returned per request.

**On June 1, 2026**, the old endpoints (**List Audit log items for Product**, **List Audit log items for Organization**)
will be redirected to the new V2 endpoints with default pagination parameters applied.

We recommend switching to the new V2 endpoints as soon as possible to avoid any unexpected behavior after the redirect takes effect.
For more details, please refer to the [Public Management API documentation](https://api.configcat.com/docs/).
