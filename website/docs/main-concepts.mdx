...
You can enable JSON validation for text settings to ensure their values are always valid JSON. When this option is turned on, ConfigCat enforces JSON validity for the setting's value.  
Your application will get a string value. You will need to parse it into JSON if necessary.
