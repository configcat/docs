...
#### Plain Values vs Predefined Variations
- **Plain Values**: Enter values directly each time you configure a feauture flag's value. Best for simple, one-off configurations or quick experiments.
+ **Plain Values**: Enter values directly each time you configure a feature flag's value. Best for simple, one-off configurations or quick experiments.
When creating a feature flag or setting, you can choose how to manage its values:

- **Plain Values**: Enter values directly each time you configure a feauture flag's value. Best for simple, one-off configurations or quick experiments.
- **Predefined Variations**: Define a set of reusable values upfront when creating the flag. Each variation has:
  - **Value** (required): The actual value returned by the flag
  - **Name** (optional): A human-readable label shown in dropdowns
  - **Hint** (optional): Context about when to use this variation

Predefined Variations ensure consistency across rules, prevent typos in complex values (like JSON configurations), and provide self-documenting configurations for your team.

:::tip
Learn more about [Predefined variations](/advanced/predefined-variations) including when to use each approach, how to convert between types, and best practices.
:::
