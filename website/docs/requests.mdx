...
team for exact requirements and performance details.
...
Typically, you have a high number of frontend/mobile/desktop application instances. Your user count determines their number.
...
the default polling interval down up to 1 second. In this case, we are calculating with 10 running instances.
...
The [ConfigCat Proxy](/advanced/proxy/proxy-overview/) allows you to host a feature flag evaluation and config JSON proxy/cache service within your own infrastructure.  
The [CDN proxy](/advanced/proxy/endpoints/#cdn-proxy) feature allows you to proxy/cache config JSON downloads, and the ConfigCat SDKs can be routed to the ConfigCat Proxy running in your own infrastructure, greatly reducing the number of config JSON downloads.

### Purchase additional quota

You can purchase an additional quota for your subscription to increase your limits. [Contact us](https://configcat.com/support/) for more information.
