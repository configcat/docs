...
You can lower the frequency your application downloads the `config JSON` by setting larger polling intervals or using a different [polling mode](./advanced/caching.mdx) other than the default auto polling. See the [SDK References for more.](/sdk-reference/overview)
