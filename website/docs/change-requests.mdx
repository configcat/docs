---
id: change-requests
title: Approval Flow & Scheduled Changes
...
The **Approval Flow** [product preferences](https://app.configcat.com/product/preferences) control which environments require approval.

You can enable it for the entire product or for specific environments.
...
A change request can be scheduled to apply the proposed changes automatically at a selected date and time. You can create a scheduled change request by clicking the down arrow next to the **Save & publish changes** or **Propose changes** button and selecting **Schedule changes**.

<img src="/docs/assets/change-request/schedule-changes-button_192dpi.png" alt="Schedule changes button location" width="1366" height="768" decoding="async" loading="lazy"/>

In the **Schedule change request** dialog, you can set when the proposed changes should be applied. A non-scheduled change request can also be converted into a scheduled one later, and a scheduled request can be unscheduled at any time, reverting it to a normal, non-scheduled change request.

<img src="/docs/assets/change-request/schedule-change-request_192dpi.png" alt="Schedule changes button location" width="1366" height="768" decoding="async" loading="lazy"/>

In approval-required environments, a scheduled change request is only applied if it is approved before the scheduled time. If approval is pending, or the change request is otherwise blocked, ConfigCat will not apply it at the planned time.

### Failed to apply on schedule

If a scheduled change request cannot be applied at the planned time (for example, because it was never approved), scheduling is disabled and the change request is reverted to a regular open change request. You can then resolve the issue and apply it manually or schedule it again.
...
- Feature flags cannot be deleted if they are referenced by an open change request (even if referenced indirectly, as prerequisites).
- Predefined variations cannot be removed if the feature flag is referenced by an open change request. 
- Segments cannot be removed if they are used in a feature flag referenced by an open change request.
