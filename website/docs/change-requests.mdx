...
A change request can be scheduled to apply automatically at a selected date and time. You can create a scheduled change request from the down arrow next to the **Save & publish changes** or **Propose changes** button and selecting **Schedule changes**.
...
In the schedule dialog, you can set when the proposed changes should be applied. A non-scheduled change request can also be converted into a scheduled one later, and a scheduled change request can be unscheduled at any time so it becomes a normal, non-scheduled change request again.
...
In approval-required environments, a scheduled change request is only applied when it has been approved. If the approval is missing or the request is otherwise blocked, ConfigCat will not apply it on the scheduled time.
