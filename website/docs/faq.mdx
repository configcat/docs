...
Yes, ConfigCat is ISO/IEC 27001:2022 for Information Security Management Systems (ISMS). Click [here](https://configcat.com/iso/) to learn more.
...
ConfigCat evaluates feature flags locally in your application. No data is sent to ConfigCat servers if you use our SDKs.
...
### Does ConfigCat collect browser data when pulling feature flags?

No. The SDK only downloads config JSON files. ConfigCat doesn't collect, store, or process any user or browser data.
