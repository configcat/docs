...
### I can't log in to ConfigCat using two-factor authentication (2FA).
...
_Solution 4:_ Authenticator app 2FA codes are time-based and require your phone's clock to be synchronized with the official time. If your phone's time is incorrect or you've recently changed time zones, switch your phone's time settings from "Manual" to "Automatic" to fix the issue.
...
Yes, you can use AND operators in your Targeting Rules. (Only available in [Config V2](./advanced/config-v2.mdx) and later)
...
If you want to use Targeting Rules based on email address, phone number, or other sensitive data, you can use the [Confidential text comparators](./targeting/targeting-rule/user-condition.mdx#confidential-text-comparators).
...
<img src="/docs/assets/cli/code-refs.png" alt="Code references screenshot" decoding="async" loading="lazy"/>
...
You can pass the current date as a [custom User Object attribute](./targeting/targeting-rule/user-condition.mdx#comparison-attribute) to the ConfigCat SDK attribute when evaluating the feature flag, and you can use that attribute with our [Date and Time comparators](./targeting/targeting-rule/user-condition.mdx#date-and-time-comparators) in your targeting rules to target specific dates.
...
### Is it possible to set up A/B/C test (33%/33%/33%) distribution with ConfigCat?
