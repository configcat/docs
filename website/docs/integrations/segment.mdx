...
let client = ConfigCatClient.get(sdkKey: "#YOUR-SDK-KEY#") { options in
    options.hooks.addOnFlagEvaluated { details in
        if details.user != null{
            analytics.identify(details.user.identifier)
        }
...
2. **Send an event to Twilio Segment** using the `Feature Flag Evaluated` event name. If the feature flag was evaluated with a user object, set the userId for the event, otherwise set a random generated string as the anonymusId. Include the following event properties:
