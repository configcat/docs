...
1. Install the ConfigCat and Amplitude SDKs to your application.
2. Configure the ConfigCat SDK with your ConfigCat SDK key and the Amplitude SDK with your Amplitude ApiKey.
3. In your code, subscribe to the `flagEvaluated` hook during the ConfigCat SDK initialization and send feature flag evaluation data to Amplitude experiments.  
There are 2 different approaches that you can use to send feature flag analytics events to Amplitude. 
    - You can send an `$exposure` tracking event when the feature flag is evaluated. 
    This way you can create custom reports based on feature flag evaluation events and use the Experiments feature in Amplitude.
...
client := configcat.NewCustomClient(configcat.Config{SDKKey: "#YOUR-SDK-KEY#",
    Hooks: &configcat.Hooks{OnFlagEvaluated: func(details *EvaluationDetails) {
         ...
         amplitude.Identify(identifyObj, amplitude.EventOptions{UserID: details.Data.User.(*UserData).Identifier})
    }}})
```
</TabItem>
...
let client = ConfigCatClient.get(sdkKey: "#YOUR-SDK-KEY#") { options in
   options.hooks.addOnFlagEvaluated { details in
      ...
      identify.set(property: "configcat_" + details.Data.Key, value: details.value)
...
The documentation only shows code examples to those languages that Amplitude natively supports and has an official SDK, 
but you can use this integration in other languages as well with sending an event to Amplitude with a third-party SDK or with using the <a href="https://www.docs.developers.amplitude.com/analytics/apis/http-v2-api/#upload-request" target="_blank" rel="noopener noreferrer">Amplitude's Upload request API</a>. 
You can subscribe to the FlagEvaluated hook in the ConfigCat SDK and send an Event to Amplitude with the `$exposure` event name and the following event properties:
1. `flag_key`: the feature flag's key from the FlagEvaluated hook's EvaluationDetails
2. `variant`: the evalated feature flag's value or the variationId from the FlagEvaluated hook's EvaluationDetails
3. `variation_id`: the evalated feature flag's value or the variationId from the FlagEvaluated hook's EvaluationDetails
4. `user_id` (optional): in case you are using the tracking in a backend component or you don't identify all your event sendings to Amplitude with user details, 
...
:::note
If you are using Text feature flags with long text values (e.g. JSON values), you can send the `variationId` as the `Variant name` to Amplitude instead of the `value`.
