...
In addition to the types mentioned above, you also have the option to use the `getAnyValue` method, where the `defaultValue` parameter can be nullable.
...
ConfigCatClient* client = [ConfigCatClient getWithSdkKey:@"#YOUR-SDK-KEY#"
    ...
    options.logger = [CustomLogger alloc];
}];
```
