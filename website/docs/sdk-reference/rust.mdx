...
| --------- | ------------------------------------------------------------------------------------------- |
| `sdk_key` | SDK Key to access your feature flags and settings. Get it from the _ConfigCat Dashboard_.   |
...
The _ConfigCat SDK_ downloads the latest config data from the ConfigCat CDN automatically and stores it in the cache. The `Duration` parameter specifies how frequent the config JSON downloads will happen.
