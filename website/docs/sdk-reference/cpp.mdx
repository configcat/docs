...
- `onConfigChanged(std::shared_ptr<const Settings>)`: This event is emitted first when the client's internal cache gets populated.
...
- `onFlagEvaluated(const EvaluationDetailsBase&)`: This event is emitted each time the client evaluates a feature flag or setting.
...
In the ConfigCat SDK, the default logger (`ConsoleLogger`) writes logs to the standard output, but you can override it with your implementation via the `logger` client option.
The custom logger must implement the `ILogger` abstract class.
