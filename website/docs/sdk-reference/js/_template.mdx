...
export const showUsingBehindProxySection = (platform) => platform === "node";
export const showBestPracticesSection = (platform) => platform === "browser";
...
export const getAdjustedToc = (platform) => {
  ...
  let adjustedToc;
  showCodePenDemoSection(platform) || (adjustedToc = removeSection(adjustedToc, toc, "working-demo-on-codepen"));
...
<If condition={showUsingBehindProxySection(props.platform)}>
  ...
  You can provide your own proxy server settings (proxy server/port) by adding a `proxy` option parameter when creating the ConfigCat client.
