...
<If condition={props.platform === "browser"}>
  ...
  :::info
  This SDK is suitable for the following types of browser applications:
...
<If condition={props.platform && canImportFromCdn(props.platform)}>
  <Tabs groupId="install">
    <TabItem value="NPM" label="via NPM">
      ...
      option to `node16`, `nodenext` or `bundler` in your `tsconfig.json`. For TypeScript versions older than 4.7, you need to fall back to
      module resolution `node` and importing from the main entry point `@configcat/sdk`.
      :::
...
<If condition={props.platform && !canImportFromCdn(props.platform)}>
  ...
  <If condition={props.platform === "node"}>
    ...
    option to `node16` or `nodenext` in your `tsconfig.json`. For TypeScript versions older than 4.7, you need to fall back to
    module resolution `node` and importing from the main entry point `@configcat/sdk`.
    :::
