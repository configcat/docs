...
- **Security**: The Proxy can act as a [server side flag evaluation](/V2/advanced/proxy/endpoints#api) component. Using it like that can prevent the exposure of *config JSON* files to frontend and mobile applications.
- **Scalability**: Horizontal scaling allows you to align with the load coming from your applications accordingly.
- **Streaming**: The Proxy provides real-time feature flag change notifications via [Server-Sent Events (SSE)](/V2/advanced/proxy/endpoints#sse) and [gRPC](/V2/advanced/proxy/grpc).
...
- **Webhook**: The Proxy has [webhook endpoints](/V2/advanced/proxy/endpoints#webhook) (for each underlying SDK) which can be set on the <a target="_blank" href="https://app.configcat.com/product/webhooks">ConfigCat Dashboard</a> to be invoked when someone saves & publishes new feature flag changes.
...
- **8050**: for standard HTTP communication. ([API](/V2/advanced/proxy/endpoints#api), [CDN proxy](/V2/advanced/proxy/endpoints#cdn-proxy), [Webhook](/V2/advanced/proxy/endpoints#webhook), [SSE](/V2/advanced/proxy/endpoints#sse))
- **8051**: for providing diagnostic data ([status](/V2/advanced/proxy/monitoring#status), [prometheus metrics](/V2/advanced/proxy/monitoring#prometheus-metrics)).
- **50051**: for [gRPC](/V2/advanced/proxy/grpc) communication.
...
You can then check the [status endpoint](/V2/advanced/proxy/monitoring#status) of the Proxy to ensure it's working correctly: `http://localhost:8051/status`
...
The **SDK identifier** part is later used in [endpoint route variables](/V2/advanced/proxy/endpoints) to recognize which SDK must serve the given flag evaluation request.
...
<td>Describes the location of your feature flag and setting data within the ConfigCat CDN. This parameter needs to be in sync with your Data Governance preferences. <a href="/docs/V2/advanced/data-governance">More about Data Governance</a>.</td>
</tr>
...
<td>-</td>
<td>The <a href="/docs/V2/advanced/notifications-webhooks/#calculating-signature">key used to sign</a> the webhook requests sent to the Proxy. <a href="/docs/V2/advanced/proxy/endpoints#webhook">More about the webhook endpoint</a>.</td>
</tr>
...
<td><code>300</code></td>
<td>The time period within the webhook requests are considered valid. <a href="/docs/V2/advanced/proxy/endpoints#webhook">More about the webhook endpoint</a>.</td>
</tr>
...
:::info
The ConfigCat Proxy supports *shared caching*, which means it can feed an external cache that is shared by other ConfigCat SDKs. You can read more about this feature and the required minimum SDK versions [here](/v2/advanced/caching/#shared-cache).
...
The Proxy doesn't create the DynamoDB table automatically, it must must already exist with a partition key called `key`. The data stored in the table has the following attributes:
- `key`: The unique cache key determined by the ConfigCat SDK within the Proxy.
...
<td>Turns the main HTTP server on/off. This server hosts the <a href="/docs/V2/advanced/proxy/endpoints#cdn-proxy">CDN proxy</a>, <a href="/docs/V2/advanced/proxy/endpoints#api">API</a>, <a href="/docs/V2/advanced/proxy/endpoints#sse">SSE</a>, and <a href="/docs/V2/advanced/proxy/endpoints#webhook">webhook</a> endpoints.</td>
</tr>
<tr>
<td>
...
<td><code>false</code></td>
<td>Turns the hosting of the <a href="/docs/V2/advanced/proxy/monitoring/#status">status endpoint</a> on the main HTTP port (default: <code>8050</code>) on/off.</td>
</tr>
...
The options for HTTP endpoints are discussed in the [Endpoints](/V2/advanced/proxy/endpoints) section.
