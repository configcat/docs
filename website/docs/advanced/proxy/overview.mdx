...
This means that you can set up your ConfigCat SDK instances to use the Proxy as data source.
...
To protect the used SDK keys from public exposure, within the Proxy, they are mapped to unique SDK identifiers. 
...
- ### 1. Automatic configuration with Proxy Profiles
  ...
  This way it can react to config/environment changes on the fly without a restart.
  ...
  - #### Select which SDK Keys your Proxy should use
    ...
    Select those config/environment pairs whose SDK key you want to make available for your Proxy instance.
  ...
  - #### Locate SDK identifiers for the selected SDK Keys
    You can find the SDK identifiers generated for the selected config/environment pairs on the ConfigCat Dashboard right where you would find their SDK Key.
    ...
    On the opening dialog, you can find the SDK identifier of the actual config/environment pair's SDK key for each Proxy Profile.
  ...
  - #### (Optional) Set up how your Proxy should be notified about feature flag changes
    There are two ways how a Proxy can detect feature flag changes. Each config JSON behind each SDK key is automatically polled for changes in a configured frequency. 
    ...
    - ##### Config JSON poll interval 
      ...
      Click on the configure icon under the `Connection preferences` column, set the poll interval and click on `SAVE POLL INTERVAL`.
    ...
    - ##### Webhook notification
      ...
      Click on the configure icon under the `Connection preferences` column and select `Webhook notification`.
      Then, set your Proxy instance's public URL and click `ADD PROXY URL`.
  ...
  - #### All configuration options related to Proxy Profiles
    ...
    <td>-</td>
    <td>The Proxy Profile's secret used by the Proxy to authenticate requests to the Public Manangement API.</td>
    </tr>
    <tr>
    <td>
    ...
    <td>The base URL from where the Proxy will fetch the SDK Keys selected in the connected Proxy Profile.</td>
    </tr>
    <tr>
    <td>
    ...
    <td>300</td>
    <td>The polling rate (in seconds) of how frequently the Proxy will fetch for SDK Key changes in the connected Proxy Profile.</td>
    </tr>
    <tr>
    <td>
    ...
    <td>300</td>
    <td>The time period (in seconds) within webhook requests are considered valid. 
    <a href="#webhook-notification">More about how to set up webhooks for a Proxy Profile</a>.</td>
    </tr>
    <tr>
    <td>
    ...
    <td>The verbosity of the Proxy Profile related logs.<br />Possible values: <code>error</code>, <code>warn</code>, <code>info</code>, or <code>debug</code>.</td>
    </tr>
    <tr>
    <td>
    ...
    <td>The CDN base url (forward proxy, dedicated subscription) from where the ConfigCat SDKs will download their config JSON.</td>
    </tr>
    <tr>
    <td>
    ...
    <td><code>warn</code></td>
    <td>The verbosity of the logs generated by the ConfigCat SDK SDKs spawned for the connected Proxy Profile.<br />Possible values: <code>error</code>, <code>warn</code>, <code>info</code>, or <code>debug</code>.</td>
    </tr>
    </tbody>
    </table>
...
- ### 2. Manual configuration
  ...
  <td><code>300</code></td>
  <td>The time period (in seconds) within webhook requests are considered valid. <a href="/docs/advanced/proxy/endpoints#webhook">More about the webhook endpoint</a>.</td>
  </tr>
