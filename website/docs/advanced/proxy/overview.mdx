...
- **Security**: The Proxy can act as a [server side flag evaluation](/docs/v2/advanced/proxy/endpoints#api) component. Using it like that can prevent the exposure of *config JSON* files to frontend and mobile applications.
- **Scalability**: Horizontal scaling allows you to align with the load coming from your applications accordingly.
- **Streaming**: The Proxy provides real-time feature flag change notifications via [Server-Sent Events (SSE)](/docs/v2/advanced/proxy/endpoints#sse) and [gRPC](/docs/v2/advanced/proxy/grpc).
...
- **Webhook**: The Proxy has [webhook endpoints](/docs/v2/advanced/proxy/endpoints#webhook) (for each underlying SDK) which can be set on the <a target="_blank" href="https://app.configcat.com/product/webhooks">ConfigCat Dashboard</a> to be invoked when someone saves & publishes new feature flag changes.
...
- **8050**: for standard HTTP communication. ([API](/docs/v2/advanced/proxy/endpoints#api), [CDN proxy](/docs/v2/advanced/proxy/endpoints#cdn-proxy), [Webhook](/docs/v2/advanced/proxy/endpoints#webhook), [SSE](/docs/v2/advanced/proxy/endpoints#sse))
- **8051**: for providing diagnostic data ([status](/docs/v2/advanced/proxy/monitoring#status), [prometheus metrics](/docs/v2/advanced/proxy/monitoring#prometheus-metrics)).
- **50051**: for [gRPC](/docs/v2/advanced/proxy/grpc) communication.
...
You can then check the [status endpoint](/advanced/proxy/monitoring#status) of the Proxy to ensure it's working correctly: `http://localhost:8051/status`
