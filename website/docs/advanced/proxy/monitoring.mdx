...
The ConfigCat Proxy provides diagnostic data via a `/status` endpoint and via a Prometheus compatible `/metrics` endpoint.
...
:::note
You can enable the status endpoint on the main HTTP port (default: `8050`) with the [HTTP configuration options](/docs/v2/advanced/proxy/proxy-overview/#http).
:::
