...
You can check the migration status on the ConfigCat Dashboard [here](https://app.configcat.com/v2/migration-status).

### Step 5: Clean up

Once you are sure that all your users are upgraded to the new version of your application (i.e. the [migration status page](https://app.configcat.com/v2/migration-status) 
reports that the V1 configs are not downloaded for a long while), you can complete the migration process by deleting the V1 config. 
This will prevent confusion and accidental use of the old config.
