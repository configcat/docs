...
You can check the migration status on the ConfigCat Dashboard under the migration status page that is available from the V2 config's context menu in the sidebar.
...
To verify that the V1 configs haven't been accessed for a significant period, check the migration status page accessible via the V2 config's context menu in 
