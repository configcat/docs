...
The tool also takes care of creating the corresponding ConfigCat products and environments, and it will do everything
...
After the translation phase, the import tool loads the result of that into ConfigCat. This is when the corresponding
ConfigCat product is actually created (or updated if it has already existed).
...
* If you see success icons next to every project, it means that everything went fine, no significant issues were
  detected.
...
be more simpler and easier to understand, at the expense of fewer possibilities for fine-tuning access control.
...
It can easily happen though that you want to add a user to more than one product. In such cases, invite the user to
...
### Adjust a code base that use LaunchDarkly SDK
...
1. Instead of a LaunchDarkly client instance, obtain a ConfigCat one:
   ...
   Please note that ConfigCat client doesn't maintain a persistent connection to the remote server, but use different
...
1. Adjust the wait-for-initialization logic.
   * When using Auto Polling mode, you can rewrite it like this:
     ...
     for initialization internally anyway. (It's worth noting though that if initialization can't complete within the
...
The conversion can be done in a similar way on other platforms as well. Obviously, there can be minor differences, plus
advanced topics are not covered here, so it's recommended to start this work with going through the SDK reference for
your platform.

### Adjust a code base that use OpenFeature

You may have an easier time if feature flagging is done using the [OpenFeature](https://openfeature.dev/docs/reference/intro/)
