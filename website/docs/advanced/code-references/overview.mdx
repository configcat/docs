...
<img width="400" src="/docs/assets/cli/scan/code_ref.png" alt="ConfigCat code references button" />
...
:::info
If the command line argument is not a suitable option for you to pass the patterns, you can set the `CONFIGCAT_ALIAS_PATTERNS` environment variable to a comma delimited list of patterns. The CLI reads that environment variable during the scan execution.
...
The regex pattern must include at least one capture group that will be treated as the actual alias. When more then one group is defined, the first one is selected.
To bind the actual feature flag keys to aliases, the CLI replaces a `CC_KEY` placeholder with the known keys.
...
:::note
The CLI implicitly adds the optional `` ` ``, `'`, `"` wrapping around the flag keys, so you don't have to add them manually in your pattern around the `CC_KEY` placeholder.
